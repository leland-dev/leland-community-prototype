...
export default function IncredibleHomePage() {
  useEffect(() => {
    document.title = "Leland — Do Ambitious Things";
  }, []);
...
type Path = {
  ...
  icon: any;
  cover: string;
  /** gradient that washes the card on hover */
  glow: string;
  /** tint color used for the icon chip */
  chipBg: string;
  sub: string[];
};
...
const PATHS: Path[] = [
  {
    key: "admissions",
    name: "Admissions",
    tagline: "Top MBAs, law, med, GMAT.",
    icon: GraduationCap,
    cover: bootcamp2,
    glow: "from-amber-400/55 via-amber-500/20",
  ...
  },
  {
    key: "ai",
    name: "AI Leader",
    tagline: "Build, ship, lead the future.",
    icon: Rocket,
    cover: aiBuilder1,
    glow: "from-emerald-400/55 via-emerald-500/20",
  ...
  },
  {
    key: "career",
    name: "Career",
    tagline: "Land it. Lead it. Level up.",
    icon: Briefcase,
    cover: bootcamp1,
    glow: "from-sky-400/55 via-sky-500/20",
  ...
  },
  {
    key: "life",
    name: "Life",
    tagline: "Own it. Live ambitious.",
    icon: Heart,
    cover: lelandPlus1,
    glow: "from-rose-400/55 via-rose-500/20",
...
function PathCard({
  ...
  return (
    <motion.div
      onMouseEnter={() => onHover(path.key)}
      onMouseLeave={() => onHover(null)}
      animate={{
        scale: isHover ? 1.025 : isDimmed ? 0.97 : 1,
        opacity: isDimmed ? 0.6 : 1,
        y: isHover ? -8 : 0,
      }}
      transition={{ type: "spring", stiffness: 280, damping: 28 }}
      className="group relative aspect-[4/5] cursor-pointer overflow-hidden rounded-3xl border border-white/15 shadow-[0_30px_80px_-30px_rgba(0,0,0,0.75)]"
    >
      {/* background image */}
      <motion.img
        src={path.cover}
      ...
      {/* themed accent that blooms on hover */}
      <div
        className={`absolute inset-0 bg-gradient-to-t ${path.glow} to-transparent opacity-0 transition-opacity duration-500 group-hover:opacity-100`}
      />

      {/* icon chip top-left */}
      <div className="absolute left-5 top-5">
        <motion.div
          animate={{ rotate: isHover ? 6 : 0, scale: isHover ? 1.08 : 1 }}
          transition={{ type: "spring", stiffness: 300, damping: 22 }}
          className={`inline-flex h-12 w-12 items-center justify-center rounded-2xl border border-white/25 ${path.chipBg} backdrop-blur-md`}
        >
      ...
      <div className="absolute inset-x-5 bottom-5 text-left">
        <h3
          ...
          style={{ fontSize: "clamp(32px, 3.4vw, 52px)", lineHeight: 0.95 }}
        >
          {path.name}
        </h3>
        ...
        >
          <motion.div
            ...
            transition={{ duration: 0.25, delay: isHover ? 0.1 : 0 }}
            className="pt-4"
          >
              ...
              {path.sub.map((s) => (
                <li key={s}>
                  ...
                  >
                    ...
                    <ArrowRight
                      size={12}
                      className="opacity-50 transition-opacity group-hover/sp:opacity-100"
                    />
                  </button>
                </li>
              ))}
...
function Hero() {
  ...
  return (
    ...
    >
      ...
      {/* Hero body — top-aligned, path tiles dominate */}
      <div className="relative z-10 mx-auto flex w-full max-w-[1400px] flex-1 flex-col items-center px-6 pt-5 pb-8 text-center md:px-12 md:pt-8 md:pb-10">
        ...
        {/* 4 PATH CARDS — the focal element */}
        <motion.div
          initial={{ opacity: 0, y: 20 }}
          animate={{ opacity: 1, y: 0 }}
          transition={{ duration: 0.7, delay: 0.35 }}
          className="mt-8 grid w-full grid-cols-1 gap-4 sm:grid-cols-2 md:mt-10 md:gap-5 lg:grid-cols-4"
        >
          {PATHS.map((path) => (
            <PathCard
              key={path.key}
              path={path}
              isHover={hoveredPath === path.key}
              isDimmed={hoveredPath !== null && hoveredPath !== path.key}
              onHover={setHoveredPath}
              onPick={setSearchValue}
            />
          ))}
        </motion.div>

        {/* Universal search offramp — shrunken, anchored to the bottom */}
        <motion.div
          initial={{ opacity: 0, y: 12 }}
          animate={{ opacity: 1, y: 0 }}
        ...
        >
          ...
          <div className="flex items-center gap-2 rounded-full border border-white/25 bg-white/[0.08] px-3 py-2 shadow-[0_10px_30px_-12px_rgba(0,0,0,0.55)] backdrop-blur-md transition-colors focus-within:border-[#15b078]/55 focus-within:bg-white/[0.12]">
            <Sparkles size={16} className="ml-1 shrink-0 text-[#9ff2c8]" />
            <input
              value={searchValue}
            ...
            <button
              aria-label="Send"
              className="inline-flex h-8 w-8 items-center justify-center rounded-full bg-[#15b078] text-white shadow-md transition-colors hover:bg-[#12a06c]"
            >
              <ArrowRight size={14} />
            </button>
          </div>
