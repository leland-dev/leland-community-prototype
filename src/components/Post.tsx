...
export function PostImageGrid({
  ...
  return (
    <div className={`relative ${className}`}>
      ...
      <button onClick={() => scroll("left")} className="absolute left-2 top-1/2 z-0 -translate-y-1/2 flex h-8 w-8 items-center justify-center rounded-full bg-white/60 text-gray-dark shadow-sm backdrop-blur-sm transition-colors hover:bg-white/85">
        <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><polyline points="15 18 9 12 15 6" /></svg>
      </button>
      <button onClick={() => scroll("right")} className="absolute right-2 top-1/2 z-0 -translate-y-1/2 flex h-8 w-8 items-center justify-center rounded-full bg-white/60 text-gray-dark shadow-sm backdrop-blur-sm transition-colors hover:bg-white/85">
