...
export default function TopNavClassic() {
                      ...
                      {navMenuOpen && (
                        <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: "auto", opacity: 1 }} exit={{ height: 0, opacity: 0 }} transition={{ duration: 0.2 }} className="overflow-hidden">
                          <div className="ml-[22px] border-l-[1.5px] border-gray-stroke pl-2">
                            ...
                            {/* Toggle the classic feed + post-detail frame between
                                centered (1280) and edge-to-edge (window edges). */}
                            <button
                              onClick={() => { setFeedEdgeToEdge(!feedEdgeToEdge); setProfileOpen(false); }}
                              className="flex w-full items-center gap-[10px] rounded-lg p-3 text-[14px] font-medium text-gray-dark hover:bg-[#222222]/5"
                            >
                              {feedEdgeToEdge ? (
                                <svg className="h-5 w-5 shrink-0 text-gray-dark" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.75" strokeLinecap="round" strokeLinejoin="round"><rect x="6" y="4" width="12" height="16" rx="1.5" /><path d="M3 8v8" /><path d="M21 8v8" /></svg>
                              ) : (
                                <svg className="h-5 w-5 shrink-0 text-gray-dark" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.75" strokeLinecap="round" strokeLinejoin="round"><rect x="3" y="4" width="18" height="16" rx="1.5" /><path d="M9 4v16" /><path d="M15 4v16" /></svg>
                              )}
                              {feedEdgeToEdge ? "Center feed (1280)" : "Feed to screen edges"}
                            </button>
                          </div>
                        </motion.div>
                      )}
